...
Once we select the desired time period and area via the filters, we can sort the items that match our search by ID, Date and Time, or select the number of resulting items we are interested in per page
...
When a specific `Item` sparks our interest, clicking on it will bring us to detailed overview page for that selected `Item`. <br>
...
* **Copy URL**: We can also retrieve the the unique URL of the specified `Asset`, which allows us directly integrating the link into programmatic workflows, wihthout the need to download the `Asset`. We can get it by simply clicking on the *Copy URL* option under the `Asset` of our interest.
