...
For many years, using Earth Observation (EO) data meant downloading large files to the computer. Cloud computing changes this completely by bringing the data and the tools to process it together. This allows for large-scale analysis that was not possible before. Now, it is possible to:
...
![Figure _ COG structure. Retrieved from CNG](img/cogtiff.png)
...
![Figure _ Zarr structure. Retrieved from CNG.](img/zarr1.png)
...
- Working with raster data (like images or elevation models) that does not change frequently.
