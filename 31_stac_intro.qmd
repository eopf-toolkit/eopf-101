...
Welcome to the chapter on EOPF and STAC. In the following section, we will introduce you to the the **S**patio-**T**emporal **A**sset **C**atalog (STAC). We will explain its fundamental principles and, most importantly, we will explore its structure and core components. Understanding the fundamentals of STAC is key in order to be able effectively discover and access data from STAC catalogs.
...
Let us start with the *Drinks* a category themselves. The menu is analog to a STAC `Catalog`, as it serves as the top-level entry point, providing an overview of all beverages available.<br>
