...
This section introduces the readily available datasets re-engineered as part of the ESA's Earth Observation Processing Framework (EOPF) initiative. 
Accessible through the [EOPF Sentinel Sample Service's STAC Catalog](https://stac.browser.user.eopf.eodc.eu/?.language=en), these datasets are undergoing continuous re-processing from the Sentinel-1, Sentinel-2, and Sentinel-3 missions. This ongoing effort aims to provide users with cloud-optimised versions of crucial Earth Observation data. 

## By the end of this page, we will be able to...

- 🛰️ Identify the re-engineered Sentinel Missions by ESA.
- 📖 Get an overview to the [EOPF Sentinel Sample Service's STAC Catalog](https://stac.browser.user.eopf.eodc.eu/?.language=en)
...
## What's next?
In the following chapter, we will dive deeper into the advantages of the `zarr` data format and we start with practically working with EOPF Zarr datasets.
