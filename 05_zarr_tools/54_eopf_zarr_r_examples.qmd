...
- 📊 How to format and visualise satellite data on a curvillinear grid
...
As described in the previous R section, Zarr data arrays are often packed or compressed in order to limit space, and may need to be scaled or offset to their actual physical units or meaningful values. 

This information is contained in the metadata associated with the Zarr store. In the previous R section, we created a helper function to obtain these values, setting the `offset` to 0 and `scale` to 1 if they do not need to be offset or scaled.
...
get_scale_and_offset(
  l2_ocn_url,
  "owi/S01SIWOCN_20250224T054940_0025_A332_160E_072A26_VV/measurements/wind_direction"
)

get_scale_and_offset(
  l2_ocn_url,
  "owi/S01SIWOCN_20250224T054940_0025_A332_160E_072A26_VV/measurements/latitude"
)

get_scale_and_offset(
  l2_ocn_url,
  "owi/S01SIWOCN_20250224T054940_0025_A332_160E_072A26_VV/measurements/longitude"
)
```
...
In this section, we accessed additional Zarr data from the [EOPF Sentinel Zarr Sample Service STAC Catalog](https://stac.browser.user.eopf.eodc.eu/?.language=en) using `rstac` and `Rarr`. We extracted measurement variables and curvillinear coordinates, formatted them using the `stars` package's `st_as_stars()` function, and visualised the data.
