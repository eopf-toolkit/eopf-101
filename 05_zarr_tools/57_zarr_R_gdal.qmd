---
title: "Access EOPF Sentinel Zarr via GDAL in `stars` and `terra`"
format: html
execute: 
  cache: refresh
---
...
In the following [notebook](./58_rstac_gdalcubes.qmd), we will explore how to create a data cube from STAC items of the [EOPF STAC Catalog](https://stac.browser.user.eopf.eodc.eu/?.language=en) using the `rstac` and `gdalcubes` packages in R.